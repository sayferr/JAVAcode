...
//        int windowSize = 5;
//
//        int startPage = Math.max(1, page + 1 - windowSize / 2);
...
//
            ...
            int endPage = Math.min(startPage + sizePage, startPage);

//            List<teacher> pageTeachers = teachers.subList(fromIndex, toIndex);
...
//        model.addAttribute("teachers",  pageTeachers);
    ...
    @ResponseBody
    @GetMapping("/path/filter/{search}")
    public List<teacher> deleteByIdAjax(@PathVariable String search) {
        List<teacher> teacher = teacherRepository.findAllByStr(search);
        return teacher;
    }
